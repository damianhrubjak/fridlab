...
<div class="bg-pallette-black group relative overflow-hidden text-white">
    <a href="{{ route('fe-pages.modelShowPage', $model->slug) }}"
    ...
    <div class="relative flex flex-col items-stretch justify-start sm:h-full sm:flex-row">
        ...
        <a class="left-0 h-full w-full sm:absolute" href="{{ route('fe-pages.modelShowPage', $model->slug) }}">
            <img src="{{ route('file-thumbnail-serve', $mainImage->slug) }}" alt="{{ $model->title }}"
                class="h-64 w-full object-cover sm:h-full sm:w-3/4">
        </a>
