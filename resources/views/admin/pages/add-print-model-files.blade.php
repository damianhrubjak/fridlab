...
@section('page-title', 'Pridať súbory pre model - ' . $printModel->title)
    ...
    <div class="mt-20 flex h-full w-full flex-col items-center justify-center">
        ...
        @if (Session::get('success'))
            <div class="mb-5 w-4/5 bg-green-700 bg-opacity-50 py-2 px-5 text-2xl">
                <div class="flex items-center justify-between">
                    <p class="font-bold text-white">Súbory boli úspešne pridané!</p>
                    <i class="fa-solid fa-circle-check text-white"></i>
                </div>
                <div class="flex items-center justify-center">
                    <a href="{{ route('admin-pages.print-model-files.index', $printModel->slug) }}"
                        class="block text-base font-bold text-white underline">Späť na súbory modelu
                        {{ $printModel->title }}</a>
                </div>
            </div>
        @endif
        ...
        <div class="flex w-4/5 flex-col">
            ...
            <form enctype="multipart/form-data"
                action="{{ route('admin-pages.print-model-files.store', $printModel->slug) }}" method="POST">
                @csrf
